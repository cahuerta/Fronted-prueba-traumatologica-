  ...
  return (
    <div style={s.wrap}>
            ...
            {actual.opciones?.map((op, i) => {
              ...
              return (
                <div key={i} style={{ ...s.opcion, ...(esCorrecta ? s.opcionCorrecta : {}) }}>
                  ...
                  {(estado === "discusion" || estado === "cerrada") && (
      ...
      <div style={s.controles}>
        ...
        <span style={s.totalVotos}>{resultados.total} voto(s)</span>
      </div>

      {(estado === "votando" || estado === "discusion") && detalle.length > 0 && (
        <div style={s.detalleBox}>
          <h3 style={s.h3}>Quién votó qué (para pedir fundamento oral)</h3>
...
const s = {
  ...
  opcionVotos: { background: "#4FC3D9", color: "#0E1526", fontWeight: 700, fontSize: 13, borderRadius: 20, padding: "2px 12px" },
  ...
  controles: { display: "flex", alignItems: "center", gap: 16, marginBottom: 24 },
  ...
  totalVotos: { color: "#94A3B8", fontSize: 13 },
  detalleBox: { background: "#16213A", border: "1px solid rgba(244,241,233,0.12)", borderRadius: 12, padding: 18, maxWidth: 500 },
